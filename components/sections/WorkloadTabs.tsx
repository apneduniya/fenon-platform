...
export function WorkloadTabs({ items, cardLabel, linkLabel }: WorkloadTabsProps) {
  return (
    <Tabs defaultValue={items[0].id} orientation="vertical" className="mt-[25px] grid gap-0 lg:grid-cols-[320px_1fr] lg:gap-[60px]">
      <TabsList activateOnFocus variant="line" className="grid h-auto w-full grid-cols-3 items-stretch gap-[13px] rounded-none p-0 lg:flex lg:flex-col lg:gap-0">
        {items.map((item, i) => (
          <TabsTrigger
            key={item.id}
            value={item.id}
            className="group/tab h-12 flex-none justify-between gap-0 rounded-none border-0 border-b border-border px-0 text-tab font-normal text-muted-foreground after:hidden group-data-vertical/tabs:justify-between data-active:text-foreground lg:h-[75px] lg:group-data-vertical/tabs:justify-start"
          >
      ...
      {items.map((item) => (
        <TabsContent key={item.id} value={item.id} keepMounted className="grid gap-[23px] text-base lg:grid-cols-[1fr_440px] lg:gap-[47px]">
          ...
          <div className="rounded-lg border border-border bg-muted px-6 pt-[26px] pb-[14px] lg:mt-[18px]">
            ...
            <div className="mt-5 flex justify-between border-t border-border pt-[17px] lg:mt-[29px] font-mono text-mono-xs text-muted-foreground">
